...
1. **Server-side Yjs**: The `EditorRoom` DO needs to maintain a `Y.Doc` in memory. `partyserver` supports this (stateful DO), but we need to handle the Yjs sync protocol manually on the server (encode/decode updates via `y-protocols/sync`). Alternative: use `y-partyserver` if it exists as a helper, or implement the ~30 lines of sync protocol handling directly.

2. **Initial state bootstrapping**: When `EditorRoom` has no persisted Yjs state, it needs to initialize from the current Lexical JSON in D1. This requires converting Lexical JSON → Yjs doc server-side (`$generateYjsDocFromJSON` or equivalent). Check if `@lexical/yjs` exports this for non-browser environments, or if we need a simpler approach (e.g. store plain text in Yjs `Y.XmlText`, let the first connecting client seed the doc).

3. **Field-level granularity overhead**: Many small Durable Objects vs. fewer larger ones. Cloudflare charges per-request to DOs, not per-DO-instance. Since DOs are created on-demand (user focuses a field) and idle quickly, this should be cheap. But worth monitoring.

4. **Iframe boundary**: `InlineLexicalEditor` renders inside an iframe (`useFrame`). The WebSocket connection should be opened from the parent frame (where React Query and auth context live), not inside the iframe. The `useCollaborativeEditor` hook should live in the parent, with the Yjs provider passed down.
