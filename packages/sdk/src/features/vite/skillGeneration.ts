...
// Frontmatter must start at byte 0 of the file or markdown formatters will
// treat the `---` lines as thematic breaks and insert blank lines around them.
// So we inject the notice *after* the closing `---` instead of prepending it.
function injectNoticeAfterFrontmatter(source: string): string {
  if (!source.startsWith("---\n")) return `${NOTICE}\n\n${source}`;
  const closeIdx = source.indexOf("\n---\n", 4);
  if (closeIdx === -1) return `${NOTICE}\n\n${source}`;
  const endOfClose = closeIdx + "\n---\n".length;
...
function generateSkillContent(skillName: string, appRoot: string): string {
  ...
  return injectNoticeAfterFrontmatter(substituted);
}
